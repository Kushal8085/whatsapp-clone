...
const initializeSocket = (server) => {
  const io = new Server(server, {
    cors: {
      ...
      credentials: true, //cookies are stored in the browser and sent with requests to the server, allowing for session management and authentication. Server checks that whether the token is included in the cookie or not by this option
  ...
  //io = whatsapp server
  //socket = one user
  //io.on = Runs when a new user connects (Gives you that user's socket)
  //socket.on = Server listens to an event coming from that user
  //socket.emit = Send data to ONLY that specific user
  // io.emit = Send data to ALL connected users
